...
tidyData<- subset(aData, select = grep("mean|std|type",names(aData)))
names<-names(tidyData)
tidyData<-cbind(aData[,2],tidyData)
names(tidyData)<-c("activity","type",names[-1])
...
means<-aggregate(tidyData[,-(1:2)], by=list(activity = tidyData$activity, subject=tidyData$type), mean)
